...
my $primers_R1_blast = $seq_dir.'/blast/out.blast/'. $R1_id.'.blast';
my $primers_R2_blast = $seq_dir.'/blast/out.blast/'. $R2_id.'.blast';
...
for my $file (@files){
    ...
    while(<FILE>){
      chomp;
	     my @array = split /\,/,$_;
	     if($#array>5){
	        my ($read,$template,$str_on_read,$end_on_read,$str,$end,$evalue,$btop) = @array[0,1,2,3,4,5,6,8];
...
sub assign_category(){
	...
	while(<FILE>){
	    ...
	    if(exists($R1->{$nm."_Frd"}) && exists($R2->{$nm."_Rvs"}) && exists($R1->{"DBU1-primer"}) && exists($R2->{"DBD2-primer"})){

        #print Dumper $R1;

         if($R1->{'PS1.0-primer'}->{end} < $R1->{$nm."_Frd"}->{str} && $R1->{$nm."_Frd"}->{str} < 100 &&
		   $R2->{'PS2.0-primer'}->{end} < $R2->{$nm."_Rvs"}->{str} && $R2->{$nm."_Rvs"}->{str} < 100){
