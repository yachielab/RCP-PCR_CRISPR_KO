...
parser.add_argument('-in','--input_files', action='store_true', default=False,nargs='+',help='Input files fastq files (both pair-end files)')
parser.add_argument('-t','--targets', action='store_true', default=False,nargs='+',help='Input target informtion in csv format. (see wiki for detail)')


parser.add_argument('-out','--output_name', action='store_true', default="output" ,nargs='?')
parser.add_argument('-r','--ratio', type=int, help='Minimum threashold (0 < ratio < 0.5 ) to call mutation profile',default=0.1,nargs='?')
parser.add_argument('-c','--core_num', type=int, help='Number of cores for multi-processing on local computer.',default=1,nargs='?')
parser.add_argument('-sge','--sge_computing', type=int, help='1 if computing on SGE computers.',default=0,nargs='?')
